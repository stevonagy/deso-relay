...
// Clean consolidated version: search + comments (pagination, nested, counters, collapse), no duplicate declarations.
...
// Normalize various inputs into { usernameOrPk, type }
const normalizeSearch = (raw: string): { usernameOrPk: string; type: 'username' | 'publicKey' } => {
  let q = (raw || '').trim();
  q = q.replace(/^@+/, '');
  try {
    if (/^https?:\/\//i.test(q)) {
      ...
      const uIndex = parts.findIndex(p => p.toLowerCase() === 'u');
      if (uIndex !== -1 && parts[uIndex+1]) q = parts[uIndex+1];
      else if (parts.length) q = parts[parts.length - 1];
    }
  } catch {}
  const isLikelyPk = /^[1-9A-HJ-NP-Za-km-z]{25,80}$/.test(q) && /[0-9]/.test(q) && /[A-Za-z]/.test(q);
  return { usernameOrPk: q, type: isLikelyPk ? 'publicKey' : 'username' };
};
  ...
  while (stack.length) {
    ...
    if (Array.isArray(cur)) cur.forEach(v => stack.push(v));
      ...
      for (const [k, v] of Object.entries(cur)) {
        if (k === 'PublicKeyBase58Check' && typeof v === 'string') out.add(v as string);
        if (v && (typeof v === 'object' || Array.isArray(v))) stack.push(v as any);
      }
    }
  }
...
export default function FeedScreen() {
  ...
  const [userSearchResults, setUserSearchResults] = useState<Post[]>([]);
  const [userSearchHandle, setUserSearchHandle] = useState<string>('');
  ...
  const [commentsPageSize, setCommentsPageSize] = useState<Record<string, number>>({}); // per-post page size
  const [collapsedChildren, setCollapsedChildren] = useState<Record<string, boolean>>({}); // per-comment collapse state
  ...
  useEffect(() => { loadRecent(); }, [loadRecent, nodeBase]);
  ...
  function getCommentChildren(raw: any): any[] {
    if (!raw) return [];
  ...
  // Fetch posts authored by a given username/public key and show them inline
  async function loadUserPosts(input: string) {
    try {
      setUserSearchActive(true);
      setUserSearchLoading(true);
      setUserSearchError(null);
      setUserSearchResults([]);
      const ns = normalizeSearch(input);
      let pk = ns.usernameOrPk;
      let handle = ns.usernameOrPk;
      if (ns.type === 'username') {
        handle = ns.usernameOrPk;
        try {
          const prof = await postJson(nodeBase + '/api/v0/get-single-profile', {
            Username: ns.usernameOrPk,
            NoErrorOnMissing: true,
          });
          pk = prof?.Profile?.PublicKeyBase58Check || '';
        } catch {}
      }
      const req = ns.type === 'username'
        ? { Username: handle, ReaderPublicKeyBase58Check: publicKey || '', NumToFetch: 50, MediaRequired: false }
        : { PublicKeyBase58Check: pk, ReaderPublicKeyBase58Check: publicKey || '', NumToFetch: 50, MediaRequired: false };
      const resp = await postJson(nodeBase + '/api/v0/get-posts-for-public-key', req);
      const posts: Post[] = (resp?.Posts || resp?.PostsFound || []).map((raw: any) => mapPost(raw)).filter(Boolean);
      setUserSearchResults(posts);
      setUserSearchHandle(handle);
    } catch (e: any) {
      setUserSearchError(e?.message || 'Greška pri dohvaćanju postova.');
    } finally {
      setUserSearchLoading(false);
    }
  }

  // Comments: load for a given parent post
  async function loadCommentsFor(parentHashHex: string) {
    ...
    try {
      ...
      const resp = await postJson(nodeBase + '/api/v0/get-single-post', {
  ...
  const followingNewCount = useMemo(() => {
    ...
    return cnt;
  }, [recentPosts, blocked, followingSet]);
  ...
  const MenuModal = () => {
    ...
    const chooseNode = (url: string) => { setNodeBase(url); close(); };
...
;
  ...
  const feedTabs = (
    <View style={{ paddingHorizontal: 16 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
        ...
        <Pressable onPress={() => { setSearchOpen(v => !v); setTimeout(() => inputRef.current?.focus(), 50); }} style={{ marginLeft: 'auto', padding: 8 }}>
        ...
        <View style={{ marginTop: 10 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <TextInput
              ...
              onSubmitEditing={() => { const q = (searchQuery || '').trim(); if (!q) return; loadUserPosts(q); setSearchOpen(false); }}
            ...
            />
            <View style={{ width: 8 }} />
            <Button title="Search" onPress={() => { const q = (searchQuery || '').trim(); if (!q) return; loadUserPosts(q); setSearchOpen(false); }} />
          </View>
        </View>
      )}
    </View>
  ...
  const renderItem = ({ item }: { item: Post }) => {
    ...
    const commentCount = typeof p?.CommentCount === 'number' ? p.CommentCount : undefined;
        ...
        {commentsOpen[item.PostHashHex] && (
          <View style={{ marginTop: 12 }}>
            {commentsLoading[item.PostHashHex] ? (
              ...
              <Text style={{ color: colors.sub }}>Nema komentara.</Text>
            ) : (
              <View style={{ gap: 10 }}>
                {commentsByPost[item.PostHashHex].map((cmt) => {
                  ...
                  const children = getCommentChildren(cp);
                  const hasManyKids = Array.isArray(children) && children.length > 3;
                  const collapsed = collapsedChildren[cmt.PostHashHex] ?? true;
                  const visibleKids = hasManyKids && collapsed ? children.slice(0, 3) : (children || []);
                  return (
                    <View key={cmt.PostHashHex} style={{ padding: 10, borderWidth: 1, borderColor: colors.border, borderRadius: 10 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        {!!cpk && (
                          <Image source={{ uri: (Deso as any).getProfilePicUrl ? (Deso as any).getProfilePicUrl(cpk) : `${nodeBase}/api/v0/get-single-profile-picture/${cpk}` }} style={{ width: 22, height: 22, borderRadius: 11, backgroundColor: colors.border, marginRight: 8 }} />
                        )}
                      ...
                      <View style={{ marginTop: 8 }}>
                        ...
                        />
                        <Button
                          title={busy === busyKey ? 'Replying…' : 'Reply'}
                          onPress={async () => {
                            if (!publicKey) return Alert.alert('Login required', 'Please log in to reply.');
                            ...
                            setBusy(busyKey);
                            try {
                              const unsigned = await (Deso as any).replyUnsigned({ updaterPublicKey: publicKey, parentPostHashHex: cmt.PostHashHex, body });
                              await signAndSubmit(unsigned);
                      ...
                      {/* CHILD COMMENTS (one level) */}
                      {Array.isArray(visibleKids) && visibleKids.length > 0 && (
                        <View style={{ marginTop: 8, marginLeft: 16, gap: 8 }}>
                          {visibleKids.map((kid: any) => {
                            const kp = kid;
                            const khandle = (Deso as any).usernameOrPk?.(kp) ?? (kp?.ProfileEntryResponse?.Username ? '@' + kp.ProfileEntryResponse.Username : (kp?.PosterPublicKeyBase58Check || ''));
                            const kpk: string = kp?.PosterPublicKeyBase58Check || kp?.ProfileEntryResponse?.PublicKeyBase58Check || '';
                            const kimgs: string[] = kp?.ImageURLs || kp?.BodyObj?.ImageURLs || [];
                            const kidHash = kp?.PostHashHex;
                            const replyVal = commentReplyText[kidHash] ?? '';
                            const kidBusy = kidHash + ':reply';
                            return (
                              <View key={kidHash} style={{ padding: 8, borderWidth: 1, borderColor: colors.border, borderRadius: 8 }}>
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                  {!!kpk && (
                                    <Image source={{ uri: (Deso as any).getProfilePicUrl ? (Deso as any).getProfilePicUrl(kpk) : `${nodeBase}/api/v0/get-single-profile-picture/${kpk}` }} style={{ width: 18, height: 18, borderRadius: 9, backgroundColor: colors.border, marginRight: 6 }} />
                                  )}
                                  <Text style={{ color: colors.text, fontWeight: '700', flexShrink: 1 }} numberOfLines={1}>{khandle}</Text>
                                </View>
                                {!!kp?.Body && <Text style={{ color: colors.text, marginTop: 4 }}>{kp.Body}</Text>}
                                {!!kimgs?.length && <Image source={{ uri: kimgs[0] }} style={{ width: '100%', height: 120, borderRadius: 6, marginTop: 4 }} resizeMode="cover" />}
                                <View style={{ marginTop: 6 }}>
                                  <TextInput
                                    value={replyVal}
                                    onChangeText={(t) => setCommentReplyText(prev => ({ ...prev, [kidHash]: t }))}
                                    placeholder="Reply to comment…"
                                    placeholderTextColor={colors.sub}
                                    style={{ borderWidth: 1, borderColor: colors.border, color: colors.text, borderRadius: 8, padding: 6, marginBottom: 4 }}
                                    multiline
                                  />
                                  <Button
                                    title={busy === kidBusy ? 'Replying…' : 'Reply'}
                                    onPress={async () => {
                                      if (!publicKey) return Alert.alert('Login required', 'Please log in to reply.');
                                      const body = (commentReplyText[kidHash] || '').trim();
                                      if (!body) return;
                                      setBusy(kidBusy);
                                      try {
                                        const unsigned = await (Deso as any).replyUnsigned({ updaterPublicKey: publicKey, parentPostHashHex: kidHash, body });
                                        await signAndSubmit(unsigned);
                                        setCommentReplyText(prev => ({ ...prev, [kidHash]: '' }));
                                        await loadCommentsFor(item.PostHashHex);
                                      } catch (e: any) { Alert.alert('Reply failed', e?.message ?? String(e)); }
                                      finally { setBusy(null); }
                                    }}
                                    disabled={busy != null || !(commentReplyText[kidHash] || '').trim()}
                                  />
                                </View>
                              </View>
                            );
                          ...
                          {hasManyKids && (
                            <Pressable onPress={() => setCollapsedChildren(prev => ({ ...prev, [cmt.PostHashHex]: !collapsed }))} style={{ paddingVertical: 4 }}>
                              <Text style={{ color: colors.accent, fontWeight: '700' }}>{collapsed ? `Show ${children.length - 3} more repl${children.length - 3 === 1 ? 'y' : 'ies'}` : 'Show less'}</Text>
                            </Pressable>
                          )}
                        </View>
                      )}
  ...
  // Visible list (derived only once here)
  ...
  const visible = mode === 'recent' ? recentFiltered : followingFiltered;
// Loading state
      ...
      {error ? (
        ...
        <View style={{ flex: 1 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 8 }}>
            <Text style={{ fontWeight: '600', fontSize: 16, marginRight: 8, color: colors.text }}>
              Rezultati za @{userSearchHandle}
            </Text>
            <View style={{ marginLeft: 'auto', flexDirection: 'row' }}>
              <Pressable
                onPress={() => { setUserSearchActive(false); setUserSearchResults([]); setUserSearchError(null); }}
                style={{ paddingHorizontal: 8, paddingVertical: 4 }}
              >
                <Text style={{ color: colors.accent }}>Zatvori</Text>
              </Pressable>
            </View>
          </View>
          {userSearchLoading ? (
            ...
            <Text style={{ color: colors.sub, padding: 16 }}>{userSearchError}</Text>
          ) : userSearchResults.length === 0 ? (
            <Text style={{ color: colors.sub, padding: 16 }}>Nema postova.</Text>
          ) : (
            <FlatList
              data={userSearchResults}
              keyExtractor={(it, i) => it.PostHashHex || String(i)}
              renderItem={renderItem}
              refreshControl={<RefreshControl refreshing={false} onRefresh={() => loadUserPosts(userSearchHandle)} />}
