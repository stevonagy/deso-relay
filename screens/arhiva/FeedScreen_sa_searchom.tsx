...
// Accepts: "@user", "user", "https://deso.com/u/user", "bitclout.com/u/user", public key (Base58)
const normalizeSearch = (raw: string): { usernameOrPk: string; type: 'username' | 'publicKey' } => {
  let q = (raw || '').trim();
  // strip leading @
  q = q.replace(/^@+/, '');
  // If it's a full profile URL, extract the last path segment
  try {
    if (/^https?:\/\//i.test(q)) {
      const url = new URL(q);
  ...
  // DeSo public keys are Base58Check, usually length 50-60 and mix of [1-9A-HJ-NP-Za-km-z]
...
// Robustly parse following PKs from get-follows-stateless
...
export default function FeedScreen() {
  ...
  // User search results (posts by a specific user)
  ...
  async function loadUserPosts(input: string) {
    try {
      ...
      // Resolve username -> public key if needed
      if (ns.type === 'username') {
        ...
        if (!pk) {
          setUserSearchError('Korisnik nije pronađen.');
          setUserSearchLoading(false);
          return;
        }
      }
      
// Get posts for that public key or username (most recent)
        ...
        <View style={{ flex: 1 }}>
          ...
          {userSearchLoading ? (
            ...
            <FlatList
              ...
              onRefresh={() => loadUserPosts(userSearchHandle)}
              refreshing={userSearchLoading}
